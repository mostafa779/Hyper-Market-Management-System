...
public class Seller {
    ...
    public boolean addToOrder(String category, String product, int quantity, double bill, boolean orderIsFinished) {
        ...
        try {
            ...
            //rs2=dc.executeQuery("select quantity from stock where name='"+product+"'");

//            if(rs2.getInt("quantity")-quantity>=0)//this statement already handeled in the gui
//            {
            if (rs.next()) {
            ...
            if (resultSet.next()) {
                ...
                double b = bill + resultSet.getDouble("bill");
...
//    Vector search(String s) {
//        Vector v = new Vector();
//        try {
//            ResultSet Rs = dc.executeQuery("select * from orders where id='" + s + "' limit 1");
//
//            if (Rs.next()) {
//                v.add(Rs.getString("id"));
//                v.add(Rs.getString("order_id"));
...
//    }
    ...
    boolean search(DefaultTableModel dft,int order_id) {
